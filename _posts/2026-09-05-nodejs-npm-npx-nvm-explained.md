...
这条命令做了什么：npx 发现项目本地和全局都没有 `create-vite`，就把它**下载到一个专用缓存目录**（Windows 是 `%LocalAppData%\npm-cache\_npx`，macOS/Linux 是 `~/.npm/_npx`）里运行。

这里要纠正一个广泛流传的误解：**npx 运行完并不会删除下载的包**——缓存会原样保留（实测：执行 `npx cowsay` 结束后，`npm-cache\_npx\` 下的 cowsay 目录还在原地），下次再执行同样的命令会直接复用缓存，速度快很多。所以 npx 的"临时"指的不是"用完删文件"，而是**不留依赖痕迹**：包不写进任何项目的 package.json、不占用项目的 node_modules、不做全局安装污染 PATH——它只是静静躺在缓存里，你完全不用管它；想清理的话把整个 `_npx` 目录删掉即可，下次用到时会重新下载。

npx 的查找顺序：当前项目的 `node_modules` → 全局已安装的包 → npx 缓存，都没有才联网下载；命中缓存就不重复下载（但每次运行仍会向仓库核对一次版本，详见下面的案例）。所以它还有两个实用功能：
...
### 案例：`npx @deepseek-ai/dsh web` 到底是什么

拿一条真实命令把本节知识全部用起来。DeepSeek 官方的 Agent 工具 **DeepSeek Harness**（对标 Codex / Claude Code 的智能体框架，"一切皆插件"架构，MIT 开源，目前还是开发者预览版）推荐的首选体验方式就是：

```bash
npx @deepseek-ai/dsh web
```

拆解这条命令：`@deepseek-ai/dsh` 是包名（DeepSeek 组织的作用域包）；`web` 是传给它的参数，意思是**启动 Web 界面**——它会在本机 `127.0.0.1:3080` 起一个 Web 服务，你再用浏览器打开这个地址，就能在网页里使用这个 Agent 工具。

围绕它回答几个最常被问到的问题：

- **装到哪里去了？** 哪儿都没"装"——包被下载到了 npx 缓存目录（`%LocalAppData%\npm-cache\_npx`），你的项目目录和全局环境不留任何痕迹。
- **怎么关闭？** 它是在终端前台运行的服务进程：回到那个终端按 `Ctrl + C` 就结束了。**只关浏览器标签页不算关闭**——那只是断开页面，服务进程还在后台跑，3080 端口仍被占用。
- **关闭后下载的包会变吗？** 不会。被杀掉的只是进程，缓存里的包原封不动，一个字节都不会变。
- **下次启动输什么？** 还是同一条命令 `npx @deepseek-ai/dsh web`——这次直接命中缓存，不再下载，启动快得多。
- **下次启动会自动更新吗？** 联网时**会**（已在 npm 11.9.0 上实测确认）。不指定版本号时，npx 每次启动都会向仓库核对 `latest`，缓存版本落后于 latest 就自动重装新版——实测把缓存里包的版本号改旧，再次运行立即被恢复成最新版；更新是在**同一个缓存目录里原地替换**，不会越攒越多。两个例外：断网时 npx 默认**直接报错**（实测 ECONNREFUSED），并不会悄悄用旧缓存，纯离线运行要加 `--offline`；不想自动更新就把版本钉死，如 `npx @deepseek-ai/dsh@0.1.5-rc.1 web`。注意这是当前新版 npm 的行为，npm 6 时代的老 npx 曾有"不检查更新"的 bug（[npm/cli#2329](https://github.com/npm/cli/issues/2329)），网上的老资料因此说法不一。
- **前置条件**：官方要求 Node.js ≥ 22.19（或 24+），版本太低会被拒绝启动——版本不够正好用上一节的方法解决：`nvm install 22 && nvm use 22`。
- **如果 npx 卡住转圈**：这个包拖了一大堆插件子包，npm 的依赖解析可能陷入长时间计算（社区已有反馈）。替代方案：`pnpm dlx @deepseek-ai/dsh web`，或者干脆 `npm i -g @deepseek-ai/dsh` 全局安装后用 `dsh web` 启动。
...
| npx | 包临时执行器 | 下载到缓存直接运行，不留依赖痕迹，缓存可复用 |
